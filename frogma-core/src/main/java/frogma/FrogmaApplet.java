	...
	public void run(){
		...
		while(!stop){
			...
			if(state == STATE_GAME){
				...
				if(renderX+screenW>gameLevel.getSolidWidth()*8)renderX=gameLevel.getSolidWidth()*8-screenW;
				if(renderY<0)renderY=0;
				if(renderY+screenH>gameLevel.getSolidHeight()*8)renderY=gameLevel.getSolidHeight()*8-screenH;
