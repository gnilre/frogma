...
import java.awt.Image;
  ...
  *
  * <p>Title: BigShyGuy </p>
  * <p>Description:  BigShyGuy class.
  * A big version of the shyguy monster. It walks until it collides with something,
  ...
  * @version 1.0
  */
public class BigShyGuy extends DynamicObject {
	...
	private int framesSinceCrunch = 0;
        ...
        public static int DEATHOFFSET=5;
	 ...
	 * @param sce
	 * We decide what kind of static object we collide with and
	 * act upon it.
	 */
	 ...
	 * @param dce
	 * @param collRole
	 * We decide if we're the invoker or the affected party,
	...
	public void collide(DynamicCollEvent dce, int collRole){
		if(collRole == DynamicCollEvent.COLL_AFFECTED){
			...
			if(invoker instanceof Player)  {
				...
				}else{ System.out.println("Undefined collision between Monster and Player. ERROR 30"); }
			}
	...
	public void advanceCycle(){
		...
		if(!isDying){
			...
			if(collBottom){
				if(collLeftRight){
					this.velX = -velX;
					if(velX>0){ this.direction = LEFT; }else{ this.direction = RIGHT; } // Make sure we're facing the direction we're walking.
					if(!onGroundPrevCycle){
						if(this.direction == LEFT){ this.velX = monsterSpeed; } else { this.velX = -monsterSpeed; }
					}
				}else{
					if(DEBUG){
			...
			if(velX>0){ this.direction = LEFT; }else{ this.direction = RIGHT; }
