...
public class ModuleLoader {
    ...
    public static Module read(DataInput dataInput) throws IOException, IllegalArgumentException {
        ...
        for (int n = 0; n < 128; n++) {
            ...
            if (sequence.numberOfPatterns < sequence.patternOrder[n])
                sequence.numberOfPatterns = sequence.patternOrder[n];
        }
        ...
        for (int m = 0; m < 64; m++) {
            for (int c = 0; c < numChannels; c++) {
                ...
                if (patt.effectCommand[m][c] == 0xE) {
                    ...
                    if (patt.effectCommand[m][c] == 0xD)
                        patt.effectValue[m][c] = nibbleDecimal2Bin((byte) patt.effectValue[m][c]);
                }
            }
        }
        return patt;
